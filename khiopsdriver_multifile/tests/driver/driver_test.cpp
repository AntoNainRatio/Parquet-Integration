...
int test_driver_fopen_errors() {
	...
	for (const char* arg : args) {
		MultiFile* mf = (MultiFile*)driver_fopen(arg, 'r');
		if (mf != nullptr) {
			failed++;
		}
	}
...
int test_driver_fread_all_file_lorem() {
	std::string path = "multifile://C/Users/KXFJ3896/Documents/Parquet-Integration/khiopsdriver_multifile/tests/files/titi.txt";

	MultiFile* mf = (MultiFile*)driver_fopen(path.c_str(), 'r');
	if (mf == nullptr) {
	...
	size_t read_size_in_loop = 10;
	while (total_read < total_read_target) {

		size_t manage_end = std::min(read_size_in_loop, total_read_target - total_read);
		code = driver_fread(buffer, sizeof(char), manage_end, mf);
		if (code != -1) {
			total_read += code;
		}
		else {
...
int test_driver_fread_all_file() {
	std::string path = "multifile://C/Users/Public/khiops_data/samples/AccidentsMedium/Users_medium.txt";
	...
	while (total_read < total_read_target) {
		...
		if (code != -1) {
			...
			if (code == 0) {
				dump_multifile(mf);
				if (feof(mf->current_handle)) {
					std::cout << "EOF reached without noticing it." << std::endl;
				}
			}
			total_read += code;
		}
...
int test_file_size() {
	...
	const char* path = "multifile://C/Users/KXFJ3896/Documents/Parquet-Integration/khiopsdriver_multifile/tests/files/titi.txt";
	int code = driver_getFileSize(path);
	int exp = strlen("Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.");
	if (code != exp) {
		print_file_size_error(path,exp,code);
		failed++;
	}

	path = "multifile://C/Users/KXFJ3896/Documents/Parquet-Integration/khiopsdriver_multifile/tests/files/file*.txt";
	...
	path = "multifile://C/Users/Public/khiops_data/samples/AccidentsMedium/Users_medium.txt";
	code = driver_getFileSize(path);
	exp = 115600676;
	if (code != exp) {
		print_file_size_error(path, exp, code);
...
int main() {
	...
	failed += test_driver_fread_all_file_lorem();
