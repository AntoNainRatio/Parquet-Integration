...
void worker(StorageBackend& input_backend, StorageBackend& output_backend, const std::string& parquet_path, JobQueue& queue, std::mutex& chunk_mutex, std::string& prefix, std::string& output_dir) {

    auto infile = input_backend.openInput(parquet_path);
...
int parquetToCsv(const char* parquet_file, const char* output_dir, const char* prefix) {
	...
	std::unique_ptr<StorageBackend> input_backend = select_backend(parquet_file);
    std::unique_ptr<StorageBackend> output_backend = select_backend(output_dir);

    try {

		std::shared_ptr<arrow::io::RandomAccessFile> infile = input_backend->openInput(parquet_file);
		...
		std::string parquet_file_string = parquet_file;
        ...
        for (int i = 0; i < num_threads; i++) {
            threads.emplace_back(worker,
                ...
                std::ref(queue),
                std::ref(chunk_mutex), 
                std::ref(prefix_string), 
                std::ref(output_dir_string));
        }
