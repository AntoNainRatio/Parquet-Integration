...
def get_multifile_path(temp_dir: str, prefix: str):
    ...
    return f"{temp_dir.replace('\\','/')}/{prefix}.txt"
...
def convert_parquet_dictionary(parquet_dictionary, temp_dir, verbose=False):
    ...
    for key, val in parquet_dictionary.items():
        ...
        if info != 0:
            return None
        merged_prefix = filename+"_merged"
        parquet_utils.merge_csv_files(temp_dir.encode('utf-8'), prefix_b, os.path.join(temp_dir,merged_prefix+".txt").encode('utf-8'))
        result[key] = get_multifile_path(temp_dir,merged_prefix)
    return result
...
def trainpredictor_parquet(dict, dict_name, data_table, target_variable, additional_data_tables,
    ...
    merged_prefix = filename+"_merged"
    merged_path = os.path.join(temp_dir_b,(merged_prefix+".txt").encode('utf-8'))
    parquet_utils.merge_csv_files(temp_dir_b, prefix_b, merged_path)

    convert_data = get_multifile_path(temp_dir, merged_prefix)
...
if __name__ == "__main__":
    ...
    parquet_utils.merge_csv_files.argtypes = [c_char_p,c_char_p, c_char_p]
    ...
    #########################################################
    #          MERGE MULTI WITHOUT DOUBLE QUOTE             #
    #########################################################

    data_table = "multifile://c/Users/Public/khiops_data/samples/AccidentsMedium/Accidents_output.txt"
    results_dir = os.path.join(
        os.path.dirname(__file__),
        "results_no_double_quote_multi")
    
    vehicles_data_file = "multifile://c/Users/Public/khiops_data/samples/AccidentsMedium/Vehicles_output.txt"
    users_data_file = "multifile://c/Users/Public/khiops_data/samples/AccidentsMedium/Users_medium_output.txt"
    places_data_file = "multifile://c/Users/Public/khiops_data/samples/AccidentsMedium/Places_output.txt"
